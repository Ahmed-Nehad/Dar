...
export default function AttendanceGrid({ monthKey }: { monthKey: string }) {
  // 1. Parse Key
  const [yearStr, monthStr] = monthKey.split('-');
  const sessions = getMonthSessions(parseInt(yearStr), parseInt(monthStr) - 1);

  // 2. Fetch Data
  const students = useLiveQuery(() => db.students.filter(s => s.is_active).toArray());
  ...
  // 4. Toggle Handler
  const toggle = async (studentId: number, dateStr: string, isPresent: boolean) => {
    if (isPresent) {
          ...
          <table className="table table-pin-rows table-pin-cols table-xs md:table-sm">
            <thead>
              <tr className="bg-base-200/50">
                ...
                {sessions.map(s => (
                  <th key={s.dateStr} className="text-center min-w-[70px] p-1 font-normal">
                    <div className="flex flex-col items-center gap-1">
                      <span className="opacity-100 text-md">{s.dayName}</span>
            ...
            <tbody>
              {students.map(student => (
                <tr key={student.id} className="hover">
                  <th className="bg-base-100 font-medium z-10">{student.name}</th>
                  {sessions.map(s => {
